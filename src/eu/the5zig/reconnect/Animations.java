...
public class Animations {
	...
	for (String name : config.getKeys()) {
		...
		animations.put(name, new Animation(
			name,
			animation.getInt("Delay Millis"),
			TimeUnit.MILLISECONDS,
			animation.getStringList("Animation").stream().map(string -> ChatColor.translateAlternateColorCodes('&', string)).collect(Collectors.toList())
			));
	    } catch (Exception e) {
